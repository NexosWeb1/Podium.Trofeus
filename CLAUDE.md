...
| Acento secundário | prata | **ouro** (CTA, preço, destaque de 1º lugar) |
...
- **Preço:** toda formatação e leitura passa por `js/price.js`. O input do painel é `type="text"`, nunca `number` (o `number` engole vírgula decimal em pt-BR e devolve string vazia sem avisar).
...
**Ouro** é **valor**: preço, botão de CTA, selo da modalidade sobre a foto, estrela do divisor, rótulos de seção e títulos do rodapé. Em fundo claro sempre `--gold-cta` (5:1); em fundo escuro sempre `--gold` (9,47:1).
...
- Card e modal mostram preço; vazio vira "Sob consulta".
- Painel: login, CRUD, upload, campo de preço, filtro, e o site reflete tudo.
